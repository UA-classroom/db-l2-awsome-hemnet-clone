      ...
      <header className="sticky top-0 z-20 border-b border-slate-100 bg-white/90 backdrop-blur">
        <div className="mx-auto flex max-w-6xl items-center justify-between px-5 py-4">
          ...
          <nav className="flex items-center gap-2">
            ...
            {isAuthenticated ? (
              <div className="flex items-center gap-2 rounded-full bg-emerald-50 px-2 py-1 text-sm font-semibold text-emerald-700">
                <span className="rounded-full bg-white px-3 py-1">
                  {username || (userId ? `User ${userId}` : 'Logged in')}
                </span>
                <button
                  className="rounded-full px-3 py-1 text-emerald-700 transition hover:bg-white"
                  onClick={logout}
                  type="button"
                >
                  Logout
